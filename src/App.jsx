...
import { Linkedin, Mail, ArrowRight, Briefcase, Code2, Award, Rocket, Terminal, Zap } from "lucide-react";
...
export default function Portfolio() {
    ...
    return (<div className="min-h-screen bg-zinc-950 text-white overflow-x-hidden">
      {/* Scroll Progress */}
      <motion.div className="fixed top-0 left-0 right-0 h-1 bg-gradient-to-r from-orange-500 via-amber-500 to-yellow-500 origin-left z-50" style={{ scaleX: scrollYProgress }}/>

      {/* Ambient Background */}
      <div className="fixed inset-0 pointer-events-none opacity-30">
        <motion.div className="absolute top-1/4 left-1/4 w-96 h-96 bg-orange-600/20 rounded-full blur-3xl" animate={{
            scale: [1, 1.2, 1],
            opacity: [0.2, 0.3, 0.2],
        }} transition={{ duration: 8, repeat: Infinity }}/>
        <motion.div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-amber-600/20 rounded-full blur-3xl" animate={{
            scale: [1.2, 1, 1.2],
            opacity: [0.3, 0.2, 0.3],
      ...
      <section className="min-h-screen flex items-center justify-center px-6 relative">
        <div className="max-w-7xl w-full">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="space-y-8">
            ...
            {/* Name */}
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="text-8xl md:text-9xl font-black tracking-tighter">
            ...
            {/* Description */}
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="max-w-2xl space-y-4">
            ...
            {/* Stats */}
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 }} className="grid grid-cols-3 gap-8 pt-12 max-w-2xl">
              {[
            { value: "2026", label: "Graduation" },
            { value: "5+", label: "Projects" },
            { value: "9+", label: "Technologies" }
        ].map((stat, i) => (<div key={i} className="border-l-2 border-orange-500/50 pl-4">
      ...
      <section className="py-32 px-6 relative">
        ...
        ].map((item, i) => (<motion.div key={i} whileHover={{ x: 5 }} className="flex items-center gap-3 p-4 border-l-2 border-orange-500/50 bg-zinc-900/50">
        ...
        ].map((item, i) => (<motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="border-b border-zinc-800 pb-4 last:border-0">
      ...
      <section className="py-32 px-6 relative bg-zinc-950">
        <div className="max-w-7xl mx-auto">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="mb-32 text-center">
          ...
          <div className="relative">
            ...
            const isLeft = i % 2 === 0;
            return (<motion.div key={i} initial={{ opacity: 0, x: isLeft ? -100 : 100 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8, delay: 0.2 }} className={`relative grid lg:grid-cols-2 gap-8 items-center ${isLeft ? '' : 'lg:grid-flow-dense'}`}>
                    {/* Timeline Node */}
                    <motion.div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 hidden lg:block z-10" whileHover={{ scale: 1.5 }}>
                      <div className="w-6 h-6 rounded-full bg-orange-500 border-4 border-zinc-950 shadow-xl shadow-orange-500/50"/>
                      <motion.div className="absolute inset-0 rounded-full bg-orange-500" animate={{
                    scale: [1, 2, 1],
                    opacity: [0.5, 0, 0.5]
                    ...
                    <div className={`lg:col-start-${isLeft ? '1' : '2'}`}>
                      <motion.div whileHover={{ scale: 1.02 }} className="bg-zinc-900 border-l-4 border-orange-500 p-8 shadow-2xl shadow-orange-600/10 relative group">
                        {/* Connection Line */}
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="space-y-3">
                            {exp.bullets.map((bullet, j) => (<motion.div key={j} initial={{ opacity: 0, x: isLeft ? -20 : 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: j * 0.1 }} className="flex items-start gap-3">
      ...
      <section className="py-32 px-6 relative">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="relative">
            ...
            return (<motion.div key={i} initial={{ opacity: 0, x: isLeft ? -100 : 100 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8, delay: 0.2 }} className={`relative grid lg:grid-cols-2 gap-8 items-center ${isLeft ? '' : 'lg:grid-flow-dense'}`}>
                    {/* Timeline Node */}
                    <motion.div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 hidden lg:block z-10" whileHover={{ scale: 1.5, rotate: 180 }} transition={{ duration: 0.3 }}>
                      <div className="w-6 h-6 rotate-45 bg-amber-500 border-4 border-zinc-950 shadow-xl shadow-amber-500/50"/>
                      <motion.div className="absolute inset-0 rotate-45 bg-amber-500" animate={{
                    scale: [1, 2, 1],
                    opacity: [0.5, 0, 0.5]
                }} transition={{ duration: 2, repeat: Infinity }}/>
                    </motion.div>
                    ...
                    {/* Content Card */}
                    <div className={`lg:col-start-${isLeft ? '1' : '2'}`}>
                      <motion.div whileHover={{ scale: 1.02 }} className="bg-zinc-900 border-l-4 border-amber-500 p-8 shadow-2xl shadow-amber-600/10 relative group">
                        {/* Connection Line */}
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="space-y-3">
                            {proj.bullets.map((bullet, j) => (<motion.div key={j} initial={{ opacity: 0, x: isLeft ? -20 : 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: j * 0.1 }} className="flex items-start gap-3">
                          ...
                          <div className="flex flex-wrap gap-2 pt-4">
                            {proj.tech.map((tech, j) => (<span key={j} className="px-3 py-1 bg-amber-500/10 border border-amber-500/30 text-amber-500 text-xs font-bold uppercase tracking-wider">
                                {tech}
                              </span>))}
                          </div>
                        </div>
      ...
      <section className="py-32 px-6 bg-zinc-950">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {["Java", "Python", "C++", "C", "Haskell", "SQL", "HTML/CSS", "MATLAB", "Git", "Qt", "SQLite", "Linux"].map((skill, i) => (<motion.div key={i} initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} whileHover={{ scale: 1.05, borderColor: "rgb(249, 115, 22)" }} className="bg-zinc-900 border-2 border-zinc-800 p-6 text-center font-bold text-lg hover:bg-zinc-800 transition-all cursor-default">
